            ...
            if(isset($response->order)){
                ...
                if($codigoHttp >= 200 && $codigoHttp <= 299){
                    ...
                    if(in_array($data->event_type,['orders.notification','orders.fulfillment_issues.resolved'])){
                        ...
                        if (isset($response->order->payment->payment_detail->promotions->details) && count($response->order->payment->payment_detail->promotions->details)) {
                            ...
                            foreach ($arrItemsPromo as $itemPromo) {
                                ...
                                if(is_array($itemPromo->discount_items) && count($itemPromo->discount_items)){ // PROMOCIONES EN IETM
                                    ...
                                    foreach ($itemPromo->discount_items as $item) {
                                        ...
                                        $promoItem[] = [
                                            'external_promotion_id' => $itemPromo->external_promotion_id ?? $itemPromo->promotion_uui,
                                            'type' => $itemPromo->type,
                        ...
                        $createOrder = MpFunctionController::createMpOrder(new Request([
                            ...
                            'json_desc_subtotal' => isset($discountsSubTotal) ? [$discountsSubTotal] : null
                        ]));
